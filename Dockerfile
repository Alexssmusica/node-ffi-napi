...
WORKDIR /app
RUN npm i @breush/ffi-napi

WORKDIR /app/node_modules/@breush/ffi-napi/ 
RUN npm i -g node-gyp node-gyp-build node-addon-api prebuildify && npm i node-gyp node-gyp-build node-addon-api @alexssmusica/ref-napi && rm -r prebuilds 
